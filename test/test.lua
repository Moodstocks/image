...
function test.test_inplaceload()
  ...
  tester:assertError(
    function() image.loadJPG(dst, image_path_jpg, 3, 'double') end,
    'Should not be able to specify different dst and tensortype'
  )
  tester:assertError(
    function() image.loadPNG(dst, image_path_png, 3, 'double') end,
    'Should not be able to specify different dst and tensortype'
  )
  tester:assertError(
    function() image.loadPPM(dst, image_path_ppm, 3, 'double') end,
    'Should not be able to specify different dst and tensortype'
  )
end
